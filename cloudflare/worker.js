  ...
  async fetch(request) {
    ...
    if (path === '/indolearn/version.json') {
      return new Response(JSON.stringify({
        ...
        changelog: "v1.0.10: 修复 Google Drive 云端同步（OAuth 回调路由错误）"
      }, null, 2), { headers: { ...cors, 'Content-Type': 'application/json' } });
    }

    if (path === '/indolearn/app.apk') {
      const apkUrl = 'https://github.com/Haydendddda/indolearn-android/releases/download/v1.0.10/IndoLearn-v1.0.10.apk';
      try {
        ...
        return new Response(resp.body, {
          headers: {
            ...
            'Content-Disposition': 'attachment; filename="IndoLearn-v1.0.10.apk"',
